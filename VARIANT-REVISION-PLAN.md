...
## Phase 5 — open items (updated 2026-08-06 after Dustin's calls)
- **TA-6 (product):** → filed as issue #150 with the three options (second
  tag / curated `see_also` fallback / leave it). Still Dustin's call.
- **AA-2 (visual):** RESOLVED — Dustin approved; badge text moved to
  `--text-primary` (commit 6941277, merged with the branch train).
- **TA-4 residue (stale `_site/` leftovers):** Dustin declined the cleanup —
  leftovers stay; they are dev-only and harmless to production builds.
- **HSTS zone override (found at deploy verification):** the Pages header
  ships `max-age=15768000; includeSubDomains` (verified on pages.dev), but a
  Cloudflare ZONE-level Edge HSTS setting rewrites it to `max-age=0` on
  dustin.space — net effect: no HSTS stored by browsers. Fix is dashboard-only
  (SSL/TLS → Edge Certificates → HSTS): either set the zone HSTS to ~6 months
  or disable the zone toggle so the Pages header flows through.
