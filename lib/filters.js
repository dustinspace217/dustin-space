...
function readableDate(dateInput) {
	...
	// Invalid-date guard, mirroring formatExposure's non-finite guard. An
	// unparseable input (undefined, null, a typo like "2025-13-99", a Date
	// built from garbage) yields an Invalid Date whose getTime() is NaN.
	// Return the same "—" em-dash placeholder rather than letting the
	// browser's literal "Invalid Date" string leak into the rendered page.
